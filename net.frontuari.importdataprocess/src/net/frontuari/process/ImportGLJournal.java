...
public class ImportGLJournal extends CustomProcess {
	...
	protected String doIt() throws java.lang.Exception {
		StringBuilder msglog = new StringBuilder("IsValidateOnly=").append(m_IsValidateOnly)
				.append(", IsImportOnlyNoErrors=").append(m_IsImportOnlyNoErrors);
		...
		if (m_DeleteOldImported) {
			sql = new StringBuilder("DELETE FROM I_GLJournal ").append("WHERE I_IsImported='Y'").append(clientCheck);
		...
		if (test.get_ColumnIndex("SPIFileContent") > 0) {
			...
			try {
				...
				while (rsValidate.next()) {
					...
					// Buscar centro de costo (último campo si la línea tiene más de 10 partes y es
					// numérico)
					String centroCosto = "";
					if (parts.length > 10) {
						...
						if (posibleCentroCosto.matches("\\d+")) { // Solo si es numérico
							...
							if (!Description.isEmpty() && Description.endsWith(centroCosto)) {
								Description = Description.substring(0, Description.length() - centroCosto.length())
										.trim();
							}
						}
					}
					...
					// Guardar centro de costo si existe
					if (!centroCosto.isEmpty()) {
						imp.set_ValueOfColumn("User1Value", centroCosto); // Cambia "User1Value" por el nombre real de
																			// tu columna si es diferente
					}
		...
		if (test.get_ColumnIndex("SPIFileContentCXP") > 0) {
			PreparedStatement pstmtValidateCXP = null;
			ResultSet rsValidateCXP = null;
			sql = new StringBuilder("SELECT * FROM I_GLJournal ")
					.append("WHERE SPIFileContentCXP IS NOT NULL AND I_IsImported='N'").append(clientCheck)
					.append(" ORDER BY AD_Org_ID,TRUNC(DateAcct),COALESCE(BatchDocumentNo, I_GLJournal_ID::varchar), COALESCE(JournalDocumentNo, ")
					.append("I_GLJournal_ID::varchar),   C_AcctSchema_ID, PostingType, C_DocType_ID, GL_Category_ID, ")
					.append("C_Currency_ID, Line, I_GLJournal_ID");
			try {
				...
				DB.close(rsValidateCXP, pstmtValidateCXP);
				rsValidateCXP = null;
				pstmtValidateCXP = null;
			}
		}
		...
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET IsActive = COALESCE (IsActive, 'Y'),")
		...
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET AD_Client_ID = COALESCE (AD_Client_ID,")
				.append(m_AD_Client_ID).append("),").append(" AD_OrgDoc_ID = COALESCE (AD_OrgDoc_ID,")
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE C_AcctSchema_ID IS NULL AND AcctSchemaName IS NOT NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ").append(
				...
				.append("WHERE C_AcctSchema_ID IS NULL AND AcctSchemaName IS NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE C_DocType_ID IS NULL AND DocTypeName IS NOT NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE GL_Category_ID IS NULL AND CategoryName IS NOT NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
			log.fine("Set DocType=" + no);
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE C_Currency_ID IS NULL AND ISO_Code IS NOT NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE C_Currency_ID IS NULL AND ISO_Code IS NULL").append(" AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		 ...
		 * DB.executeUpdate(sql.toString(), get_TrxName()); if (no != 0) log.warning
		 * ("No CourrencyType=" + no);
		 */

		// Set/Overwrite Home Currency Rate
		sql = new StringBuilder("UPDATE I_GLJournal i ").append("SET CurrencyRate=1 ")
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append(" WHERE s.C_AcctSchema_ID=i.C_AcctSchema_ID AND s.AD_Client_ID=i.AD_Client_ID")
				.append(" AND r.C_Currency_ID=i.C_Currency_ID AND r.C_Currency_ID_TO=s.C_Currency_ID")
				.append(" AND r.AD_Client_ID=i.AD_Client_ID").append(" AND r.C_ConversionType_ID=i.C_ConversionType_ID")
				.append(" AND i.DateAcct BETWEEN r.ValidFrom AND r.ValidTo ")
				// ORDER BY ValidFrom DESC
				.append(") WHERE CurrencyRate IS NULL OR CurrencyRate=0 AND C_Currency_ID>0")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				...
				.append("WHERE C_Period_ID IS NULL").append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
			log.fine("Set Period=" + no);
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				.append("SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Period, '")
				.append("WHERE C_Period_ID IS NULL OR C_Period_ID NOT IN").append("(SELECT C_Period_ID FROM C_Period p")
				.append(" INNER JOIN C_Year y ON (y.C_Year_ID=p.C_Year_ID)")
				.append(" INNER JOIN AD_ClientInfo c ON (c.C_Calendar_ID=y.C_Calendar_ID) ")
				.append(" WHERE c.AD_Client_ID=i.AD_Client_ID")
				// globalqss - cruiz - Bug [ 1577712 ] Financial Period Bug
		...
		// Posting Type
		sql = new StringBuilder("UPDATE I_GLJournal i ").append("SET PostingType='A' ")
		...
		sql = new StringBuilder("UPDATE I_GLJournal i ").append("SET AD_Org_ID=AD_OrgDoc_ID ").append(
				"WHERE (AD_Org_ID IS NULL OR AD_Org_ID=0) AND (OrgValue IS NULL OR OrgValue='') AND AD_OrgDoc_ID IS NOT NULL AND AD_OrgDoc_ID<>0")
				.append(" AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		...
		sql = new StringBuilder("UPDATE I_GLJournal o ")
				...
				// .append(" AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND
				// I_IsImported<>'Y'")
				.append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
		 ...
		 * ) .append("WHERE C_Activity_ID IS NULL AND ActivityValue IS NOT NULL")
		 * .append(" AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'"
		 * ).append (clientCheck); no = DB.executeUpdate(sql.toString(), get_TrxName());
		 ...
		 * .append(" AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'"
		 * ).append (clientCheck); no = DB.executeUpdate(sql.toString(), get_TrxName());
		 * if (no != 0) log.warning ("Invalid User1=" + no);
		 */
		...
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET AmtSourceCr = 0 ")
				.append("WHERE AmtSourceCr IS NULL").append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
			log.fine("Set 0 Source Cr=" + no);
		sql = new StringBuilder("UPDATE I_GLJournal i ")
				.append("SET I_ErrorMsg=I_ErrorMsg||'WARN=Zero Source Balance, ' ")
				.append("WHERE (AmtSourceDr-AmtSourceCr)=0").append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
			log.warning("Zero Source Balance=" + no);

		// Accounted Amounts (Only if No Error)
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET AmtAcctDr = ROUND(AmtSourceDr * CurrencyRate, 5) ") // HARDCODED
																														// rounding
				.append("WHERE AmtAcctDr IS NULL OR AmtAcctDr=0").append(" AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
			log.fine("Calculate Acct Dr=" + no);
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET AmtAcctCr = ROUND(AmtSourceCr * CurrencyRate, 5) ")
				.append("WHERE AmtAcctCr IS NULL OR AmtAcctCr=0").append(" AND I_IsImported='N'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE))
		 ...
		 * DB.executeUpdate(sql.toString(), get_TrxName()); if (no != 0) log.warning
		 * ("Check Acct Balance=" + no);
		 */

		/* validate new fields for san simon and spi */

		/*
		 * // Set TrxOrg sql = new StringBuilder ("UPDATE I_GLJournal i ")
		 ...
		 * (log.isLoggable(Level.FINE)) log.fine("Set AD_Org_ID from SPIClientValue=" +
		 * no); sql = new StringBuilder ("UPDATE I_GLJournal i ")
		 ...
		 * .append(" AND I_IsImported<>'Y'").append (clientCheck); no =
		 * DB.executeUpdate(sql.toString(), get_TrxName()); if (no != 0) log.warning
		 * ("Invalid AD_Org_ID=" + no);
		 * 
		 * // Set AmtCreditOrDebit sql = new StringBuilder ("UPDATE I_GLJournal i ")
		 * .append("CASE WHEN TrxType = 1 THEN SET AmtSourceDr,AmtAcctDr = Amt ")
		 * .append(" ELSE SET AmtSourceCr,AmtAcctCr = Amt END ")
		 * .append("WHERE i.Amt IS NOT NULL") .append(" AND I_IsImported<>'Y'").append
		 * (clientCheck); no = DB.executeUpdate(sql.toString(), get_TrxName());
		 */
		...
		sql = new StringBuilder("SELECT SUM(AmtSourceDr)-SUM(AmtSourceCr), SUM(AmtAcctDr)-SUM(AmtAcctCr) ")
				.append("FROM I_GLJournal ").append("WHERE I_IsImported='N'").append(clientCheck);
		...
		// Count Errors
		StringBuilder msgdb = new StringBuilder("SELECT COUNT(*) FROM I_GLJournal WHERE I_IsImported NOT IN ('Y','N')")
				.append(clientCheck);
		...
		sql = new StringBuilder("UPDATE I_GLJournal ").append("SET I_IsImported='N', Updated=SysDate ")
